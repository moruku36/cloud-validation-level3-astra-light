...
from c1 import validate_config, private_path, atomic, Stop, REGION

def materialize(c, out):
    validate_config(c)
    out = private_path(out)
    out.mkdir(parents=True, exist_ok=True)
    common = {k: c[k] for k in ("account_id", "experiment_id", "operator_arn", "expires_at")}
    ...
    if c.get("kms_arn", "").startswith("arn:aws:kms:"):
        ...
        backend = dict(bucket=c["backend_bucket"], key=c["state_key"], region=REGION,
                       ...
                       allowed_account_ids=[c["account_id"]], max_retries=1)
