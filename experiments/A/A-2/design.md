...
|---|---|---|---|---|
|A|ALB＋Fargate常時2AZ＋RDS PostgreSQL同期Multi-AZ|SQL整合性、コンテナ可搬性、OS保守削減、常時容量|RDS/認証の地域障害・CPUクレジット・単一writer、固定費。基本約9.9万円、余裕込み約11.9万円|条件付き採用。予算リスクを解消できなければ承認不可|
|B|ALB＋EC2 Auto Scaling 2AZ＋同じRDS|アプリ互換、継続負荷では費用低下の余地|OS/AMIパッチ・起動時間・容量補充を専任1名で維持。基盤費は未取得のためA基盤72USDの代わりにVM/EBS計60〜110USDを仮置き、同じ他費目で約9.7〜10.6万円、余裕別|不採用。成熟したAMI運用と確定価格で有意な節約が出れば再比較。VM価格の確認不足を有利な根拠としない|
