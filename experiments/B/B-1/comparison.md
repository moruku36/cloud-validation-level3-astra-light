...
### 総合比較マトリクス（コスト・性能・運用負荷・トレードオフ）

| 比較軸 | AWS代表案 (暫定1位 / 64点) | GCP代表案 (暫定2位 / 62点) | Azure代表案 (暫定3位 / 59点) |
|---|---|---|---|
| **基本概算月額 (税込)** | **76,034 円**（予算内、+23,966円余地）<br/>※未精算差額Uあり | **135,580 円**（予算超過、+35,580円）<br/>※東京主要単価未確定 | **216,493 円**（予算大幅超過、+116,493円）<br/>※固定費のみで15.6万円 |
| **性能・レイテンシ特性** | ECS Fargate 2台 (ARM) + RDS Multi-AZ。<br/>1vCPU/2GiB×2。残1台縮退時の100RPS耐性は要検証。RDS burstableクレジット枯渇リスクあり | Cloud Run min2 + Cloud SQL Plus HA。<br/>マネージドゾーン分散、コールドスタート抑制。2vCPU/16GiBでDB性能・メモリは最も潤沢 | ACA 2台 + PostgreSQL Flexible HA。<br/>vNet統合オーバーヘッド要検証。推奨3レプリカに対し2台構成のため縮退時検証必須 |
| **運用負荷 (1名体制)** | **中**（AWS単一クラウドで完結）<br/>SES/外形監視もAWS内で統合管理。ただし独自framework認証・独自Lambda外形プローブの実装保守負担あり | **中〜高**（GCP主要部 + 外部AWS依存）<br/>Cloud Runの運用性は極めて高いが、メール(SES)・外形監視でAWSの契約/権限/請求が重複 | **高**（Azure主要部 + 外部AWS依存）<br/>App Gateway WAF v2の運用・ルール管理が重厚。外部AWS監視/SESとのマルチクラウド管理負担大 |
| **主要トレードオフ** | **【低コスト・単一管理 vs 内製運用負担】**<br/>固定費を抑え予算内に収まるが、認証・外形監視の実装保守責任を自社運用で引き受ける | **【開発・運用容易性 vs DBコスト偏重】**<br/>Cloud Runで運用容易だが、30日PITR共通仮定により高額なEnterprise Plusが必須化 | **【高可用性・手厚い機能 vs 莫大な固定費】**<br/>WAF・DBが堅牢だが、App GatewayとDBの固定費だけで予算10万円を大幅に超過 |

```mermaid
quadrantChart
    title "3クラウド代表案のトレードオフ分析 (運用容易性 vs コスト)"
    x-axis "低コスト (予算10万円内)" --> "高コスト (予算超過)"
    y-axis "運用負荷大 (内製/マルチ管理)" --> "運用容易 (マネージド主導)"
    quadrant-1 "フルマネージド・高コスト"
    quadrant-2 "理想領域 (低コスト・運用容易)"
    quadrant-3 "低コスト・内製運用負担"
    quadrant-4 "非推奨 (高コスト・運用複雑)"
    "AWS案 (7.6万円 / 単一クラウド / 独自認証・監視)": [0.26, 0.45]
    "GCP案 (13.6万円 / DB Plus突出 / AWS外部依存)": [0.55, 0.65]
    "Azure案 (21.6万円 / WAF+DB固定費過大 / AWS外部依存)": [0.86, 0.42]
```
